...
bool FResourceTransferTest::RunTest(const FString& Parameters)
{
    ...
    Source->TryAdd(TEXT("wood"), 2);Bag->TryAdd(TEXT("axe"),1);
    ...
    TestEqual(TEXT("Empty deposit no duplicate"), Storage->GetItemCount(TEXT("wood")), 2);
...
bool FResourceRejectionTest::RunTest(const FString& Parameters)
{
    ...
    Bag->TryAdd(TEXT("axe"),1);Bag->TryAdd(TEXT("stone"), 96);
