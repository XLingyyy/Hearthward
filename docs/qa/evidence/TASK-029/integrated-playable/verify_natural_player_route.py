...
def run():
    ...
    station=building.get_buildings()[0]
    report["workbench_distance_before_approach"]=pawn.get_actor_location().distance(station.get_actor_location())
    yield wait(lambda: move_toward(pawn, station, 110), 25)
    yield delay(.4)
    station_id=building.nearby_workbench()
    report["workbench_id"]=station_id.to_string()
    check("workbench_reached", report["workbench_id"]!=unreal.Guid().to_string())
    opened=screen().execute_action("page:crafting")
    report["crafting_feedback"]=str(screen().get_message())
    check("crafting_entry", opened)
