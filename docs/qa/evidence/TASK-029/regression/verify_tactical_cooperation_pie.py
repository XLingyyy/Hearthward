...
def run():
    ...
    adventure_origin=p.get_actor_location()
    ...
    check("new_campaign",save.start_new_progress());yield delay(.7)
    g=p.get_component_by_class(unreal.HearthwardGameplayComponent);st["g"]=g;st["c"]=c
    camp=c.camp.get_actor_location()

    # Encounter actors are created when adventure mode is enabled. At that moment no
    # companion camp exists yet, so their authoritative origin is the player's adventure
    # origin rather than the later test camp. Derive the probe position from that origin.
