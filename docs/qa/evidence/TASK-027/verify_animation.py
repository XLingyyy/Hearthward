...
out=Path(unreal.Paths.project_saved_dir())/('Task027/physical' if interactive else 'Task027/playtest')
...
def run():
    ...
    check('real_hero_mesh',player.mesh.skeletal_mesh_asset.get_name()=='SK_Hero_Tripo')
    ...
    foot_height=min(player.mesh.get_socket_location(b).z for b in ['0_Right_Limb_1','1_Left_Limb_1'])-(player.get_actor_location().z-90.)
    report['walking_ankle_height_above_capsule_base']=foot_height
    check('walking_support_foot_height',abs(foot_height-6.146591*180./99.802912)<3.)
    walk_pose=player.mesh.get_socket_transform('0_Right_Limb_0',unreal.RelativeTransformSpace.RTS_COMPONENT)
    yield delay(.18)
    later=player.mesh.get_socket_transform('0_Right_Limb_0',unreal.RelativeTransformSpace.RTS_COMPONENT)
    ...
    player.stop_jumping();yield delay(.38)
    ...
    before=bag.get_item_count('wood');check('second_interaction_starts',interaction.interact_nearest());stage='dig_complete';yield delay(5.6)
