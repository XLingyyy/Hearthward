...
out=Path(unreal.Paths.project_saved_dir())/'Task025';out.mkdir(parents=True,exist_ok=True)
report={'passed':False,'provider':'real-model','model':'Qwen3.5-4B Q4_K_M','runtime':'llama.cpp b10964','backend':'vulkan','gpu_layers':32,'prompt':'task025-cognition-7','cases':[],'checks':{}}
...
def run():
    ...
    check('disk_load_pending',save.load_point(pending_id));check('disk_pending_preserved',ai.get_clarification_turns()==1 and len(ai.get_player_memories())==12)
