...
def run():
    ...
    yield delay(1)
    if mode in ['faults','lifecycle','timeout']:
        ...
        elif mode=='lifecycle':from lifecycle_scenarios import scenarios
        else:from timeout_scenarios import scenarios
        yield from scenarios(check,wait,delay,w,pc,ui,p,c,a,s,store)
        return
    if mode in ['dev','heldout','cpu','b']:
...
def evaluate(w,pc,ui,p,c,a,s,store):
    ...
    for case in dataset:
        ...
        unchanged=all(store.get_item_count(i)==n for i,n in before.items()) and c.get_requested()==0 and all(c.bag.get_item_count(i)==n for i,n in before_bag.items()) and c.source.get_item_count('wood')==source_before and c.owned_durability.get('axe')==20 and len(a.get_player_memories())==2
        ...
        if expected=='rule_proposal':semantic=semantic and candidate and last['intent']=='proposal'
        ...
        if semantic and expected in ['collect','craft','craft_rope','repair']:
            identity=a.get_candidate_id();confirm_start=time.monotonic();confirmed=a.confirm_candidate(identity);confirm_seconds=time.monotonic()-confirm_start
            if confirmed:
            ...
            actual={'confirmed':confirmed,'confirm_response_seconds':confirm_seconds,'phase':str(c.get_phase()),'acquired':c.get_acquired(),'delivered':c.get_delivered(),'carried':c.get_carried(),'block':c.block_reason,'storage':{i:store.get_item_count(i) for i in before},'durability':{str(k):v for k,v in c.owned_durability.items()},'events':len(a.get_events())}
