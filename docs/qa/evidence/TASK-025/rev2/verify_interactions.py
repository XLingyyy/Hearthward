...
def run():
    ...
    yield delay(1)
    if mode in ['faults','lifecycle','interactions']:
        ...
        else:from interaction_scenarios import scenarios
...
flow=run();pending=None;start=time.monotonic();was_busy=False
def tick(dt):
    global pending,model_seen,was_busy
    try:
        if st.get('ai'):
            busy=st['ai'].is_busy()
            if was_busy and not busy:
                a=st['ai'];report.setdefault('model_trace',[]).append({'input':a.get_last_input(),'raw':a.get_last_structured_result(),'intent':a.get_last_applied_intent(),'line':a.get_npc_line(),'reason':a.get_reason_code(),'context':a.get_last_filtered_context()})
            was_busy=busy
            bucket='during' if busy else 'after' if model_seen else 'before';frame_samples[bucket].append(float(dt)*1000);model_seen=model_seen or busy
