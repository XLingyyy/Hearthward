...
def run():
    ...
    for item,count in json.loads((root/"Resources/Data/gameplay.json").read_text(encoding="utf-8"))["loadout"].items():
        fixture_bag.try_add(item,int(count))
    fixture_save=sub(unreal.HearthwardSaveSubsystem,w)
