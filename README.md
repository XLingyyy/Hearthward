...
044任务分支已通过Editor构建、相关原生18/18和真实PIE 23/23检查，证据见[验证报告](docs/qa/TASK-044/REPORT.md)。已由PR #46合入main（7ce8262）；测试仍绑定报告内源码。坠落曲线、游泳耗耐力数值、新药配方、治疗区内容与视觉强度尚未配表；代码接口不等于这些内容已配置。main的046实现新增床八小时睡眠与有限生产／刷新状态，沿用044生存时钟。新存档为schema5，旧存档采用明确迁移；请勿用旧版本覆盖新档。
...
自然地图已接通伙伴、仓储、建造、制作/维修和技能页面。靠近弟弟按 T，输入“帮我采集两份木材带回营地。”，检查任务卡并确认。伙伴原有木材点初始 16 份；玩家也可以靠近场景树木、石头和灌木按 E 采集木材、石材、草药，每次5秒获得2份。每处上限分别为12/8/4份，余量随存档保存；main的046实现营地内已登记树木耗尽后两游戏日刷新，石头／草药未加入刷新白名单，营地外沿用旧规则。营地仓储附近按 R 存取物资，按 B 打开建造目录；main的046实现可从目录进入营地管理，操作发展、分工、设施和公共口粮。地图和任务日志页面可从自然地图打开；旧开发场景敌人尚未布置到自然地图。
...
- 27项物品、10个装备槽、装备计重与耐久、100起始个人容量、无限共享仓储、食物和药品消费、材料修理。
...
界面主题、图集UV、静态布局、物品格与快捷栏位于 `Resources/UI/interface.json`；玩法内容和参数位于 `Resources/Data/gameplay.json`。建造目录、材料配方、营地范围、摆放尺寸及模型外形部件也位于同一玩法配置。main的046实现工作台I需72木材，其他建造及升级费用遵循已批准campEconomy表，营地1—8阶半径50—120米；现有场景可按E真实采集木材，也可从营地仓储取出。工作台即时配方位于 `craftingRecipes`：demo每1木材制作4箭矢、2木材制作1绳索，2木材3石材制作1石斧，2草药制作1药膏，`crafting` 配置260cm访问距离和99批上限。普通配方默认掌握，不预留材料。维修费用位于 `repairRecipes`，使用可采集木材及可制作绳索；当前原型按整件全修收费，未将缺失的装备制作表推导为正式20%费用。耐久仍沿用现有按物品类型保存的模型，不区分同类装备实例；床靠近按E完成5秒就座动作后持续休息，饱食大于0时每实玩秒恢复2%最大生命；移动结束休息，不额外扣除饱食或瞬间补满耐力。篝火靠近按E，5秒消耗1鲜肉1木材得到1烤肉；新游戏补给含3鲜肉，烤肉可用快捷键2食用。main的046实现新增绳索、金属锭、基础熟食与大餐即时／后台加工；高级蓝图、装备和专门菜单仍待047／048。新增物品、技能、任务、建筑、制作配方与地点可按稳定ID扩展，存档以ID关联。中文正文为LXGW WenKai、大标题为Noto Serif CJK SC，OFL许可证随资产提供；生成美术的来源记录在 `Resources/UI/art-provenance.json`。
...
045分支接入盾牌破防、四类近战窗口、四部位伤害、独立敌人感知、持久尸体与分区报警、感应显示和战斗快照。Editor构建、相关原生20/20、最终战斗复测2/2与PIE 27/27已通过，受测SHA和限制见[045报告](docs/qa/TASK-045/REPORT.md)。现有攻击片段按3秒播放供处决流程验证，尚无专用双人处决动画；感应当前为屏幕投影轮廓。自然地图正式敌人／巡逻路线／区域照明、弓弩弹道及装备绝对表仍需内容配置，本单没有改地图或导入新资产，已由PR #47合入main。
...
安全食物物种／自然地图源点、正式营救与故乡胜利关卡由后续内容任务接入；目前提供真实回调和有限源点接口，测试采用明确夹具。四人基本生活的预算尚不等于自然地图已可完成该循环。设施复用现有模型，高级装备／菜单未擅自配表。本单Editor构建通过，相关原生25/25、PIE65/65、工具33/33通过；受测SHA、截图和录像见[046报告](docs/qa/TASK-046/REPORT.md)。

## TASK-047 成长与装备设计候选

当前顺序进入canonical047（原稿049）。[D1—D6候选方案](docs/design/DSGN-R14-progression-equipment.md)与[完整表册](docs/planning/TASK-047/TABLES.md)覆盖60级、29技能、62物品、49配方、逐件耐久／维修及双方背包升级。设计计算32项通过，首版预算45级、满级52/87技能点；见[047报告](docs/qa/TASK-047/REPORT.md)。Owner尚未批准这些新规则，Resources与Source保持main基线，当前游戏仍使用旧装备／技能／库存语义；schema6仅为候选，当前运行仍为schema5。自然获取点、任务奖励布置和运行验收尚未完成。
