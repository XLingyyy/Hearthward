...
背包与伙伴对话的场景透显改动通过 Editor Development 构建和自然地图 16 项定向 PIE 检查，包括装备、输入、页面切换及暂停状态；实际 Slate 截图见[场景透显报告](docs/qa/evidence/TASK-031/scene-overlay/REPORT.md)。本轮使用真实页面处理函数和引擎截图，未进行物理键鼠或模型推理性能验收。
