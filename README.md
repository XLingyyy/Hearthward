...
本任务分支的自然地图提供移动、视角、背包、暂停、存档、营地房屋、建造入口，以及已建工作台的制作和维修入口。篝火、工作台可用原有配方建造并随存档恢复；目前自然图没有正式木材获取路径，正常新游戏需先具备木材才能完成建造。斧头模型随已有装备状态显示，但最终手部挂点及攻击/采集动作联动尚未完成。下表其他伙伴、战斗和任务键位主要对应旧开发场景，尚未整体接入自然地图。
...
当前可运行内容仍基于开发灰盒，伙伴和敌人使用碰撞形体；TASK-026也只使用基础形体表达树木、草、岩石和地标，菜单插画不代表三维城寨或正式森林资产已制作。TASK-027已通过PR #35合入main，将既有Tripo主角接入玩家，提供待机、行走、冲刺、起跳、下落、落地、挥击、挖掘八个基础动画。待机、行走、冲刺已改为UE自带第三人称模板经IK Retargeter重定向的动画，其余五段为本地关键帧。自然地图Shift冲刺、左键预览挥击；开发场景近战成功及E五秒资源交互分别驱动挥击和挖掘。TASK-027未新增矿点或矿石收益；本任务的临时斧头外观尚未与动画联动，伙伴与敌人的外观未替换。验证与限制见[027交接](docs/handoffs/TASK-027.md)及[来源](docs/assets/TASK-027/PROVENANCE.md)。027成果已合入main；TASK-028合并后的运行回归另见交接。导航覆盖现有开发场景的可行走表面，支持静态障碍绕行和动态障碍重建；正式大世界伙伴导航、攀爬/跳跃导航连接仍未制作。完整十小时剧情、带工具和脚部IK的完整动作系统、营地生产/设施升级、重伤救援和Shipping打包尚未完成。TASK-004 自然素材源文件由此前 77 个增至 115 个，其中本批新增冷杉和松树的 38 个源文件。026只复用了其中三张地表贴图，004模型适配与整单验收仍未完成，见[既有资源汇总](resourceSummary.md)与[新增树木来源](art_source/TASK-004/polyhaven/SOURCE.md)。020经验曲线、节点、任务、建筑、制作配方与战斗参数为独立内容配置，未替代GDD未决R项。

四件 Tripo 道具源资产（石骨斧、原始鱼竿、骨肉袋、陶罐）的 FBX、参考图和预览已存入 `art_source/TASK-004/Tripo/妙妙道具`。本任务分支已导入石骨斧，并按现有 `axe` 装备状态显示临时持有外观；鱼竿、袋和陶罐仍是未导入候选，斧头动作联动尚未完成。
...
同一 PR 在 `art_source/TASK-004/Tripo/篝火` 收录一件[静态候选模型](art_source/TASK-004/Tripo/篝火/篝火模型索引.md)的参考图、FBX 与预览。本任务分支已将静态模型用于现有可建篝火的预览、实例和读档恢复；火焰仍为静态造型，没有动态火焰或新照明玩法。

已通过 [PR #29](https://github.com/XLingyyy/Hearthward/pull/29) 合入 main 的 8 件 [Tripo 房屋建筑部件制作源资产](art_source/TASK-004/Tripo/房屋建筑部件/房屋建筑部件模型索引.md)，每件均有参考图、静态 FBX 和 PNG 预览。本任务分支已导入全部8件，在营地组合为可步行进出的房屋，并另设简单碰撞体与屋顶、地板内衬修补源模型缝隙；仍是 `TEMP_VISUAL` 候选，尚无Owner视觉验收。

5 件 [Tripo 室内家具制作源资产](art_source/TASK-004/Tripo/室内家具/室内家具模型索引.md)为绳网木床、带锁木箱、木桌、木椅和金属提灯。本任务分支已导入全部5件：木桌替换可建工作台外观，其余布置于营地房屋；床、箱、椅、灯不新增交互或储物玩法，提灯旁使用独立点光源照明。
