...
基于 UE 5.8.1 的单人第三人称生存冒险游戏，设计基线为 GDD v0.3 与已批准的 DSGN-001 本地推理修订。当前工作分支已接通本地模型 UE → llama.cpp → GGUF → Qwen3.5 4B，并具备独立灰盒验证场、原生伙伴交流面板、TASK-016 开发存档闭环、TASK-017 存档管理界面、TASK-018 玩家采集入库闭环及 TASK-019 营地仓储管理界面。

当前分支包含 TASK-012 伙伴委托、TASK-013 本地模型接入，TASK-014 独立灰盒验证场景、TASK-015 背包/伙伴UI、TASK-016 世界/知识快照及 TASK-017 存档管理/退出确认。019营地仓储管理已完成实现与验证，已按用户授权提交推送任务分支；018及此前成果已推送，均未合并到 `main`，完整游戏和打包验收未完成。
...
- 单世界共享仓储、整批真实存取、重复请求保护与旧时间线请求拒绝。营地150cm内按R打开仓储管理，查看五种现有物品、选择数量存入/取出、查看单重和余量；默认暂停，与背包/交流/存档互斥，读档撤销旧会话。
- 距离交互、目标失效/移远取消、完成条件复核及完成回调；E为灰盒临时交互键。现有伙伴夹具的木材点可由玩家采集，营地可将随身木材入库；使用同一真实库存，HUD显示数量、单重、余量和结果。
...
共享仓储已接入营地夹具的E键木材入库；现有五种物品已接入R键数量选择界面，正式营地解锁、完整物品目录与兄弟转交仍待实现。011独立消费测试仍为200cm/2木材；018采集入库夹具为150cm/五秒、采集1木材、营地整批存入木材，均为隔离参数，未形成正式经济表或建造配方。
...
TASK-019：PIE执行 `Hearthward.Companion.CreateTest`，靠近资源方块按E采集，靠近营地薄板按R管理仓储，选择物品、输入正整数，再点击存入或取出；Esc/R返回，Tab查看背包，F6使用存档面板。E五秒入库随身木材仍保留。R键和150cm范围为显式开发原型参数。
...
TASK-018最终Editor构建通过，原生2项、两轮PIE39项通过；最终镜头修正另有4项定向复测和实键入库录像。覆盖范围、问题修正与限制见 [TASK-018交接](docs/handoffs/TASK-018.md)。
TASK-019最终Editor构建、两轮PIE49项（含强制GC回归）、仓库自检及31项工具测试通过；实键选择物品、数量输入、存入/取出及Tab/F6切换、关闭后移动通过。证据与限制见 [TASK-019交接](docs/handoffs/TASK-019.md)。
