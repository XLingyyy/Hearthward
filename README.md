...
UE 5.8.1 单人第三人称生存冒险项目。当前任务分支为 `codex/TASK-022-camp-building`，堆叠于021导航分支，新增自由建造与建筑存档恢复，尚未合并 main。设计依据是 GDD v0.3、DSGN-001 与本次独立配置说明 [DSGN-002](docs/design/DSGN-002-ui-gameplay.md)。
...
纸张、插画、格子、分隔线与按钮已拆成独立组件，12页共53个组件组。`Resources/UI/layout.json` 保存位置、大小、显示状态和单独图层覆盖；编辑时子控件及点击区域跟随父组件。操作与扩展说明见 [布局编辑](Resources/UI/LAYOUT.md)。

界面主题、图集UV、静态布局、物品格与快捷栏位于 `Resources/UI/interface.json`；玩法内容和参数位于 `Resources/Data/gameplay.json`。建造目录、材料配方、营地范围、摆放尺寸及灰盒外形部件也位于同一玩法配置。当前工作台8木材、篝火4木材，营地半径12米为 PROTOTYPE_ONLY 参数；现有场景可按E真实采集木材，也可从营地仓储取出。建筑暂不提供加工、休息或生产功能。新增物品、技能、任务、建筑与地点可按稳定ID扩展，存档以ID关联。中文正文为LXGW WenKai、大标题为Noto Serif CJK SC，OFL许可证随资产提供；生成美术的来源记录在 `Resources/UI/art-provenance.json`。

## 验证与限制

当前建造验证见 [TASK-022交接](docs/handoffs/TASK-022.md) 和 [建造证据](docs/qa/evidence/TASK-022/)；导航验证见 [TASK-021交接](docs/handoffs/TASK-021.md) 和 [导航证据](docs/qa/evidence/TASK-021/)；已验收UI与玩法见 [TASK-020交接](docs/handoffs/TASK-020.md)；构建/测试命令见 [BUILD_AND_TEST](docs/qa/BUILD_AND_TEST.md)。020分支成果已于2026-09-19通过用户验收；自动化通过率不用于计算视觉还原度。
