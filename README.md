...
基于 UE 5.8.1 的单人第三人称生存冒险游戏，设计基线为 GDD v0.3 与已批准的 DSGN-001 本地推理修订。当前工作分支已接通本地模型 UE → llama.cpp → GGUF → Qwen3.5 4B，并具备独立灰盒验证场、原生伙伴交流面板及 TASK-016 开发存档闭环。

当前分支包含 TASK-012 伙伴委托、TASK-013 本地模型接入，TASK-014 独立灰盒验证场景、TASK-015 背包/伙伴UI及 TASK-016 世界/知识快照；尚未合并到 `main`，完整游戏和打包验收未完成。
...
TASK-016的Editor最终构建与原生2项通过，两轮PIE54项通过，仓库工具31项通过；覆盖采集中回档、物资/知识一致恢复、真实HTTP废止、全保护档池、危险延后、损坏拒绝和跨PIE磁盘恢复。最后的历史上下文预算修改由最终构建/原生测试覆盖。实现提交 `17f6008b55f102936aa6c6056b0bab5326088401`，验证与源码绑定见 [TASK-016交接](docs/handoffs/TASK-016.md)。当前格式为开发schema 1，保存小型快照时同步写盘；完整世界后台保存、正式存档菜单、自动切图与实体重建尚未实现。
