...
基于 UE 5.8.1 的单人第三人称生存冒险游戏，设计基线为 GDD v0.3 与已批准的 DSGN-001 本地推理修订。当前工作分支已接通本地模型 UE → llama.cpp → GGUF → Qwen3.5 4B，并具备独立灰盒验证场、原生伙伴交流面板、TASK-016 开发存档闭环、TASK-017 存档管理界面及 TASK-018 玩家采集入库闭环。

当前分支包含 TASK-012 伙伴委托、TASK-013 本地模型接入，TASK-014 独立灰盒验证场景、TASK-015 背包/伙伴UI、TASK-016 世界/知识快照及 TASK-017 存档管理/退出确认。018采集入库实现与验证已完成，并按用户明确授权提交推送任务分支；017及此前成果已推送，均未合并到 `main`，完整游戏和打包验收未完成。
...
共享仓储已接入营地夹具的E键木材入库；正式营地解锁与全物品界面未接入。011独立消费测试仍为200cm/2木材；018采集入库夹具为150cm/五秒、采集1木材、营地整批存入木材，均为隔离参数，未形成正式经济表或建造配方。
...
TASK-018：PIE执行 `Hearthward.Companion.CreateTest`，靠近资源方块按E采集，靠近营地薄板按E入库；Tab查看背包，F6使用存档面板。
