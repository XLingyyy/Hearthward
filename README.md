...
[TASK-043 世界时间与持久状态](docs/tasks/TASK-043.md)接在已推送的 TASK-042 提交 `ab5596e` 上，形成[规则提案](docs/design/DSGN-R01-world-time-persistence.md)和[契约草案](docs/contracts/CT-TASK-043-world-time-persistence.md)：包含逐系统结算矩阵、投入与产出、敌人／自然物刷新、传送与保存边界。TASK-042此前草案已由PR #45合入main，本轮批准记录随043分支交付；TASK-043规则已获修改确认：睡眠8游戏小时并按实际时刻显示昼夜、敌人击晕等效击杀且不再苏醒、树木2日刷新，其他方案按确认生效；同代次清敌和奖励只结算一次，批准增量尚未合入main；本043设计分支中床仍为短时恢复、自然采集点仍不周期再生，本轮无玩法实现或运行验证。043产品规则已完成定案，下一单按顺序执行实际044（原稿046）的生存／救援设计。此前误开的052代码仅在独立分支，暂停推进，不作为043／044基线。

[TASK-044 生存与救援设计](docs/tasks/TASK-044.md)已获Owner采纳[六组决定及22项纸面案例](docs/design/DSGN-R04-survival-recovery.md)，覆盖饱食／回血、药品费用、耐力与暂停、倒地／环境伤害、伙伴救援和同时间结算。**D1—D6产品规则已于2026-09-26获Owner采纳，当前批准登记尚未修改玩法代码。** 043已批准子范围继续生效，052提前施工成果不进入当前设计分支。
