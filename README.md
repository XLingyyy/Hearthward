...
UE 5.8.1 单人第三人称生存冒险项目。TASK-024已由远端PR #21合并至 main（307b967）。当前 `codex/TASK-025-npc-cognition` 从该主干研发AI NPC认知、玩家可控记忆与多轮委托，025尚未合并 main。设计依据是 GDD v0.3、DSGN-001 与本次独立配置说明 [DSGN-002](docs/design/DSGN-002-ui-gameplay.md)。
...
- 未完成澄清保留原话与问题；“帮我采些木材”→“三份”可组成一个真实任务。认知、记录及待澄清对话随保存节点一起恢复，旧请求不能污染回档。
...
025记忆当前为玩家显式记录，未做自动总结或Embedding；每条最多120字，64条保存记录（含撤销记录）、最多4条有效文字约定，每次检索另取最多3条相关记录。待澄清最多4轮、800字，超出会明确要求重述，避免截断限制。实测4B模型会忽略自由文字约定；需要强制禁止采集时，必须选择“采集限制”并指定物品，普通文字约定仅作交流参考。弟弟目前能通过语言执行采集、查询库存和回忆记录；制作、修理、建造等语言委托尚未接入。R18/R20/R21的其它内容仍未闭合。
