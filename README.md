...
AI NPC vNext 候选分支为 `codex/ai-npc-vnext-rework-01`，受测源码提交 `6d1ca5e`，在 `04239f5` 的 TASK-032～039 集成栈上收录 TASK-027～040。TASK-040 加入三档 bounded ContextProjection、registry-driven capability prompt、Belief `LastEvidenceAt`、Episode coverage 与 Save schema 3 迁移。当前统一目标为 UE 5.8.2；该干净提交的默认 Unity Editor Development 构建因 `Json` 重定义失败，repo validator 0 errors、Python 31/31通过，当前提交原生测试未运行。真实模型 CTX e2e 仍为 BLOCKED/NOT_RUN。结果见 [clean-tree review](docs/qa/evidence/TASK-040/CLEAN_TREE_REVIEW.md) 与 [TASK-040 验证记录](docs/qa/evidence/TASK-040/VALIDATION.md)。地图与 AI 两条车道保持隔离；不把 TASK-026 视为已完成视觉验收。
