...
044任务分支已通过Editor构建、相关原生18/18和真实PIE 23/23检查，证据见[验证报告](docs/qa/TASK-044/REPORT.md)。已由PR #46合入main（7ce8262）；测试仍绑定报告内源码。坠落曲线、游泳耗耐力数值、新药配方、治疗区内容与视觉强度尚未配表；代码接口不等于这些内容已配置。main的046实现新增床八小时睡眠与有限生产／刷新状态，沿用044生存时钟。047任务分支新存档为schema6，旧存档采用明确迁移；请勿用旧版本覆盖新档。
...
自然地图已接通伙伴、仓储、建造、制作/维修和技能页面。靠近弟弟按 T，输入“帮我采集两份木材带回营地。”，检查任务卡并确认。伙伴原有木材点初始 16 份；玩家也可以靠近场景树木、石头和灌木按 E 采集木材、石材、草药，每次5秒按完好工具等级获得2／3／4份，消耗一次工具耐久（047分支）。每处上限分别为12/8/4份，余量随存档保存；main的046实现营地内已登记树木耗尽后两游戏日刷新，石头／草药未加入刷新白名单，营地外沿用旧规则。营地仓储附近按 R 存取物资，按 B 打开建造目录；main的046实现可从目录进入营地管理，操作发展、分工、设施和公共口粮。地图和任务日志页面可从自然地图打开；旧开发场景敌人尚未布置到自然地图。
...
- 047分支有62项物品，逐件装备身份与耐久，头／胸／腿／脚局部防护，兄弟各自100—300容量及装备槽，无限共享仓储。手套／腰带／箭袋保留旧ID和重量，作为装饰不提供护甲。
...
界面主题、图集UV、静态布局、物品格与快捷栏位于 `Resources/UI/interface.json`；玩法内容和参数位于 `Resources/Data/gameplay.json`。建造目录、材料配方、营地范围、摆放尺寸及模型外形部件也位于同一玩法配置。main的046实现工作台I需72木材，其他建造及升级费用遵循已批准campEconomy表，营地1—8阶半径50—120米；现有场景可按E真实采集木材，也可从营地仓储取出。047分支的49项即时配方位于 `craftingRecipes`，按设施种类、等级与图纸知识检查。石斧需10木材15石材；金属锭需2矿石1木材；精炼金属锭需2金属锭1精矿2木材。修理基准位于 `repairRecipes`，38件耐久装备使用独立实例，转交／仓储／放到地面／拾取保持身份与耐久。地面装备随档保存，唯一装备放下前需确认。兄弟新档各有斧、弓、头胸腿脚护甲、盾、24箭、4烤肉、3药；护符仅玩家持有。营地后勤员从初始营地提供双方独立付费的背包升级，营地2／4／6／8阶解锁150／200／250／300容量。床休息和八小时睡眠继续使用044／046结算。新增物品、技能、任务、建筑、制作配方与地点可按稳定ID扩展，存档以ID关联。中文正文为LXGW WenKai、大标题为Noto Serif CJK SC，OFL许可证随资产提供；生成美术的来源记录在 `Resources/UI/art-provenance.json`。
...
045分支接入盾牌破防、四类近战窗口、四部位伤害、独立敌人感知、持久尸体与分区报警、感应显示和战斗快照。Editor构建、相关原生20/20、最终战斗复测2/2与PIE 27/27已通过，受测SHA和限制见[045报告](docs/qa/TASK-045/REPORT.md)。现有攻击片段按3秒播放供处决流程验证，尚无专用双人处决动画；感应当前为屏幕投影轮廓。自然地图正式敌人／巡逻路线／区域照明仍需内容配置；弓弩参数和装备绝对表已在047分支接入，本单没有改地图或导入新资产，已由PR #47合入main。
...
安全食物物种／自然地图源点、正式营救与故乡胜利关卡由后续内容任务接入；目前提供真实回调和有限源点接口，测试采用明确夹具。四人基本生活的预算尚不等于自然地图已可完成该循环。设施复用现有模型；高级装备与制造表已由047批准并在047分支接入。本单Editor构建通过，相关原生25/25、PIE65/65、工具33/33通过；受测SHA、截图和录像见[046报告](docs/qa/TASK-046/REPORT.md)。

## TASK-047 成长、装备与制作维修

Owner已确认[D1—D6](docs/design/DSGN-R14-progression-equipment.md)。当前 `codex/TASK-047-equipment-recipes` 分支实现60级成长（满级65,785经验）、29技能各3级、满级52点；脱战且无动作时可免费洗点，不重置生命、耐力、耐久或奖励。击晕／杀死使用同一清敌与经验结算；重击精通提供确定伤害加成，感官三项升级分别控制持续时间、范围和冷却。
...
Editor构建、相关原生32/32、真实PIE64/64及工具33/33通过；最终源码绑定、结果与运行截图见[047报告](docs/qa/TASK-047/REPORT.md)。main仍为046基线，本分支未合并。
