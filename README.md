...
本任务分支正在实施 [TASK-028 建筑、物品、武器、防具等3D资产导入与游戏应用](docs/tasks/TASK-028.md)，状态为 **Active，部分完成**。已在自然营地接入房屋、篝火、工作台和临时斧头持有外观；防具源模型、TASK-027最终角色接口及完整验收仍缺。此进展仅属于 `codex/TASK-028-3d-assets-integration`，尚未合入 main。见 [TASK-028交接](docs/handoffs/TASK-028.md)。
...
本任务分支的自然地图提供移动、视角、背包、暂停、存档、营地房屋、建造入口，以及已建工作台的制作和维修入口。篝火、工作台可用原有配方建造并随存档恢复；目前自然图没有正式木材获取路径，正常新游戏需先具备木材才能完成建造。斧头模型随已有装备状态显示，但最终手部挂点及攻击/采集动作联动仍待TASK-027接口。下表其他伙伴、战斗和任务键位主要对应旧开发场景，尚未整体接入自然地图。
