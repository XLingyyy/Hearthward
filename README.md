...
基于 UE 5.8.1 的单人第三人称生存冒险游戏，设计基线为 GDD v0.3 与已批准的 DSGN-001 本地推理修订。当前工作分支已接通本地模型 UE → llama.cpp → GGUF → Qwen3.5 4B，并具备独立灰盒验证场及 TASK-015 原生伙伴交流面板。

当前分支包含 TASK-012 伙伴委托、TASK-013 本地模型接入，TASK-014 独立灰盒验证场景及 TASK-015 背包/伙伴UI；尚未合并到 `main`，完整游戏和打包验收未完成。
...
当前检索使用规则和词项评分，未安装独立Embedding模型；角色长期记忆与全局快捷建议生成尚未实现；建议区域明确显示不可用。原生自由输入、思考/台词及实际委托状态面板已接入同一伙伴开发夹具，完整NPC与生产地图仍待后续任务。
